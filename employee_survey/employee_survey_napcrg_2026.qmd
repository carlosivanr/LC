...
```{r}
# Subset data to PCC only
data_pcc <- data %>%
  filter(pcc_or_mdc == "practice")

data_numeric_pcc <- data_numeric %>%
  filter(pcc_or_mdc == "practice")

labels_pcc <- labels %>%
  filter("Primary Care Center (practice) or Multi-Disciplinary Center (clinic) employee?" == "practice")
```
...
```{r}
# Use the consent_complete variable
data_pcc %>%
  select(employee_consent_complete) %>%
  tbl_summary()
```
...
test <- data_pcc %>%
  select(
    record_id, consent_name, 
    employee_consent_complete, employee_survey_t1_complete,
    employee_compensation_t1_complete, practice_name)

test %>%
  select(-record_id, -consent_name, -practice_name) %>%
  tbl_summary()
...
test %>%
  filter(employee_compensation_t1_complete == "Complete") %>%
  mutate(consented = if_else(is.na(consent_name), 0, 1)) %>%
  select(consented) %>%
  tbl_summary()
...
```{r}
#| eval: false
# Read in kristen's file for amc only
kristens_amc <- read_csv("D:\\long_covid\\employee_survey\\data_qa\\amc_surveys.csv", show_col_types = FALSE)

# Kristen's files shows 33 and 3
# not sure how x_complete is defined
kristens_amc %>%
  select(x_complete) %>%
  tbl_summary() %>%
  as_tibble()

# What column maps onto x_complete
data_pcc %>%
  filter(practice_name == "AMC WISH Clinic") %>%
  select(employee_survey_t1_complete) %>%
  tbl_summary() %>%
  as_tibble()

# clinic_names  <- table(data_pcc$practice_name) %>% names()

# for (clin in clinic_names) {
#   print(clin)

#   tab <- data_pcc %>%
#   filter(practice_name == clin) %>%
#   select(employee_survey_t1_complete) %>%
#   tbl_summary() %>%
#   as_tibble()
...
data_pcc %>%
  select(practice_name, employee_survey_t1_complete) %>%
  tbl_summary(by = employee_survey_t1_complete) %>%
  add_overall()
```
...
data_numeric_pcc %<>%
  filter(record_id %in% data_pcc$record_id)
```
...
## Numeric scores
```{r}
# Currently only scores those with complete responses. If atleast one item is
