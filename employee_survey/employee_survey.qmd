...
data %>%
  ...
  mutate(age_range = factor(age_range, levels = c("Under 30", "30-45", "46-60", "Over 60", "Unknown"))) %>%
  mutate(experience_cat = factor(experience_cat, levels = c("<=1", "2-5", "6-10", ">10"))) %>%
  tbl_summary(
    by = pcc_or_mdc,
    label = list(
...
```{r, fig.height = 10, fig.width=6.5, fig.dpi = 600}
...
df %>%
  ...
  scale_y_continuous(labels = function(x) scales::percent(x, scale = 1), limits = c(0, 65)) +
...
## Numeric scores
```{r}
# Currently only scores those with complete responses. If at least one item is
# missing, that participants scores are dropped.
# For each participant, outputs an average across all 6 items.
...
# Test if there is a statistical difference between MDC and PCC
lm(AvgImplScale ~ pcc_or_mdc, data = (data %>%
  select(AvgImplScale, pcc_or_mdc))) %>%
  broom::tidy()

# Data suggest there is a statisticaly significant difference in the 
# implementation climate scores between MDC and PCC where PCC show
...
# Test to see if there is a difference between PCC and MDC
lm(confidence ~ pcc_or_mdc, data = (data_numeric %>%
  filter(record_id %in% ids) %>%
  select(confidence:effective, pcc_or_mdc))
) %>%
  broom::tidy()


lm(effective ~ pcc_or_mdc, data = (data_numeric %>%
  filter(record_id %in% ids) %>%
  select(confidence:effective, pcc_or_mdc))
) %>%
  broom::tidy()

# Analyses show that there is a difference in confidence and effectiveness
# pcc respondents score lower.
```
...
### Numeric tabulation of preferred ways to learn
...
# When comparing across both MDC and PCC clinics, none of the
# subscales are different suggesting that the practice cultures
# don't differ, unlike implementation climate.
```
...
stigma_scores <- data_numeric %>%
    ...
    mutate(n_missing = sum(is.na(c_across(everything())))) %>%
    ungroup() %>%
    mutate(stigma = ifelse(n_missing > 1, NA, stigma)) %>% # only 2 responses have n_missing . 1.
    select(stigma)
```
...
# comparison between MDC and PCC indicates that the two clinic types do not differ.
```

```{r}
#| eval: false
# version 3, more akin to the spirit of the original author's scoring approach.
data_numeric %>%
    select(prefer_not:waste_dollars) %>%
    mutate(across(c(prefer_not, irritate, particularly_difficult, little_help, waste_dollars), ~ (.x - 6) * -1)) %>%
    mutate(stigma_score = rowSums(., na.rm = TRUE)) %>%
    mutate(stigma_score = stigma_score * (66/50)) %>% # 60 is the max score on the 10-item version, 50 here with this version
    select(stigma_score) %>%
    bind_cols(., data %>% select(pcc_or_mdc)) %>%
  tbl_summary(
    by = pcc_or_mdc,
...
mcrs <- data %>% 
  filter(pcc_or_mdc == "PCC") %>%
  select(age_range, gender, experience_cat, personal_covid, practice_role_pcc, stigma)

model <- lm(stigma ~ age_range + gender + experience_cat + personal_covid + practice_role_pcc, data = mcrs)

broom::tidy(model)

tbl_regression(model)

# This subsets to 167 providers 
anova_model <- lm(
  stigma ~ practice_role_pcc, 
  data = (
    data #%>% 
      # filter(practice_role_pcc %in% 
      #   c("Advanced practice provider (NP, PA)",   "Medical Assistant/LPN", "Nurse/RN", "Physician")) %>%
      # mutate(practice_role_pcc = as.factor(practice_role_pcc))
      )
)
...
emms <- emmeans::emmeans(anova_model, ~ practice_role_pcc) %>%
  as_tibble()

# Test of marginal means
pwc <- emmeans::emmeans(anova_model, ~ practice_role_pcc) %>%
  pairs() %>%
  as_tibble() 
...
make_stigma_plot <- function(df) {
  df %>%
    ...
    geom_text(
      ...
      size = 3) +
    scale_x_discrete(labels = function(x) str_wrap(x, width = 40)) +
    scale_y_continuous(labels = function(x) scales::percent(x, scale = 1), limits = c(0, 100)) +
    coord_flip() +
    theme_minimal() +
    theme(legend.position = "bottom") +
    labs(x = "Statement", y = "Percent", fill = "Response")
...
## Numeric scoring, lower is worse, higher is better
```{r}
data_numeric %>%
  select(burnout_situation, pcc_or_mdc) %>%
  tbl_summary(
    by = pcc_or_mdc,
    type = list(burnout_situation ~ "continuous"),
...
model <- lm(stigma ~ burnout_situation,
  data = (
    data_numeric %>% select(burnout_situation) %>% bind_cols(data %>% select(stigma))
  )
)
