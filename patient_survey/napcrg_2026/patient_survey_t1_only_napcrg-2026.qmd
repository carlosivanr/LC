---
title: "AHRQ Long COVID - T1 Patient Survey Results NAPCRG 2026 Abstract"
...
```{r}
# Carlos Rodriguez PhD. CU Anschutz, Depapartment of Family Medicine
# Description:
# This qmd file is used to generate T1 results capped at February 28, 2026 for
# the 2026 NAPCRG conference.
```
...
```{r}
# These represent referrals that were contacted
total_ids <- data %>%
  distinct(record_id) %>%
  pull(record_id)

# Patients that consented before February 28 2026
ids <- data %>%
  # select(patient_consent_form_timestamp) %>%
  mutate(date = as.Date(format(patient_consent_form_timestamp, format = "%Y-%m-%d"))) %>%
  filter(date <= "2026-02-28") %>%
  pull(record_id)

# 
# data %>%
#   drop_na(patient_consent_form_timestamp) %>%
#   select(patient_consent_form_timestamp) %>%
#   arrange(desc(patient_consent_form_timestamp))
...
```{r}
data %>%
  mutate(date = format(patient_consent_form_timestamp, format = "%Y-%m")) %>%
  select(date) %>%
  drop_na() %>%
  group_by(date) %>%
  count() %>%
...
t1 %>%
  ...
  mutate(across(everything(), ~ case_match(.x, 
      ...
      "Very Often" ~ 5,
      .default = NA))) %>%
  mutate(na_counts = sum(is.na(c_across(everything())))) %>%   
...
## Mean score of "lack of understanding" scale from Illness Invalidation Inventory for MDC
...
t1 %>%
  ...
  mutate(across(everything(), ~ case_match(.x, 
      ...
      .default = NA))) %>%
  mutate(na_counts = sum(is.na(c_across(everything())))) %>%   
  mutate(score = rowMeans(across(serious_mdc:talk_mdc), na.rm = TRUE)) %>%
  mutate(score = ifelse(na_counts >= 2, NA, score)) %>%
  select(score) %>%
  tbl_summary(
    type = everything() ~ "continuous",
...
t1 %>%
  ...
  mutate(across(everything(), ~ factor(ifelse(.x == "Checked", 1, 0), levels = c(0, 1)))) %>%
  tbl_summary(
    value = everything() ~ "1",
    label = list(
