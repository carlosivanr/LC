...
data <- pull_redcap_report(Sys.getenv("LC_patient"), "148994", "label", "raw", "false")
...
# - record_ids 14-16 and 18 are test records
# - record_id 6 and 35 are the less complete rows of record_id 38
# data %<>%
#   filter(!record_id %in% 
#   c(10, 
#     14,
...
#     body = "Error in patient_accrual_report.qmd. Unreconciled duplicates detected.",
...
#   # Stop the render
#   stop("Error in .qmd file. Possible duplicated name. Check data")
# }
```

```{r}
# 2nd version of the algorithm to remove test and duplicated ids
test_ids <- c(14, 15, 16, 18)

# Identify duplicated record_ids
duplicated_ids <- 
  data %>%
...
# would indicate flawed logic for reporting purposes
...
if (mismatched_consent_values > 0) {
  ...
  # Stop the render
  stop("Error in .qmd file. Logic error in counting consented vs consent form complete. Revise code.")
}
```
...
tab_data <- data %>%
  # filter(!record_id %in% c(40, 42)) %>% # place holder for the no response/ no contact yet.
  select(record_id, enrollstatus, patient_consent_form_complete, healthrelated_social_needs_complete) %>%
  group_by(record_id) %>%
  fill(enrollstatus, patient_consent_form_complete, healthrelated_social_needs_complete, .direction = "updown") %>%
  slice_head() %>%
...
```{r}
#| eval: false
no_contact <- data %>% filter(record_id %in% c(40, 42))

enrolled_not_consented <- 
  data %>% filter(record_id %in% c(4, 22, 26, 30))
...
```{r}
# The N here is 28, whereas in the previous table, the N was 29. indicating there may be a record missing, so the denominator
# is different, but the value is still the same
enrolled_ids <- data %>% 
  filter(enrollstatus == "Enrollment completed") %>% 
  pull(record_id)

# enrolled but missing a record


# data %>%
#   filter(record_id %in% ) %>% 
#   filter(redcap_event_name == "consent_and_t1_sur_arm_1") %>%
#   select(patient_consent_form_complete) %>%
#   tbl_summary()
```
