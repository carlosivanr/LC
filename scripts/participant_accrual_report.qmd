...
get_report_data <- function(x, type) {
  ...
  formData <- list(token = .sms_token, content = "report", format = "csv",
                 report_id = x, csvDelimiter = "", rawOrLabel = type,
...
- Pre-enrolled patients: Number of unique record ids in all patient_preenroll_arm_1 rows
- Screened: Number of unique record ids in all screening_and_enro_arm1 rows
- Consent: Number of unique record ids in all consent_and_t1_sur_arm_1 rows
```{r}
